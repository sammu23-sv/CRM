...
ngOnInit(): void {
  ...
  const cachedTickets = this.tabService.getTickets();
  const cachedFilters = this.tabService.getTicketFilters();

  if (cachedTickets.length > 0) {
    this.allTickets = cachedTickets;
    this.currentTab = cachedFilters.currentTab;
    this.latestSearchText = cachedFilters.searchText;
    this.dateRange = cachedFilters.dateRange;
    this.applyAllFilters();
    return;
  }
  ...
  const apiUrl = this.userRole === 'Developer'
  ...
  this.http.get<any[]>(apiUrl, { headers }).subscribe({
    next: (res) => {
      this.allTickets = res;
      this.tabService.setTickets(res); // ✅ Cache
      this.applyAllFilters();
    },
  ...
  this.tabService.setTicketFilters({
    currentTab: this.currentTab,
    searchText: this.latestSearchText,
    dateRange: this.dateRange
  });
  this.tabService.setTickets(this.allTickets); // ✅ Optional, if modified
  if (id) {
