...
export class PaymentsComponent implements OnInit {
  ...
  ngOnInit(): void {
    ...
    const cached = this.tabService.getPaymentCache?.();
    if (cached?.data?.length) {
      this.allPayments = cached.data;
      this.currentTab = cached.filters.currentTab;
      this.latestSearchText = cached.filters.searchText;
      this.dateRange = cached.filters.dateRange;
      this.filterPayments();
      return;
    }
  ...
  fetchPayments(headers: HttpHeaders): void {
    const endpoint = `${environment.apiBaseUrl}/payments`;

    this.http.get<any[]>(endpoint, { headers }).subscribe({
      next: (res) => {
        this.allPayments = res;
        this.tabService.setPaymentCache?.({
          data: res,
          filters: {
  ...
  getColumnDefs(role: string): ColDef[] {
    return [
      ...
      ...(role !== 'Customer' ? [{ field: 'company_name', headerName: 'Company Name', flex: 1 }] : []),
      {
        field: 'amount_paid',
        headerName: 'Amount Paid', flex: 1,
        valueFormatter: p => `₹${parseFloat(p.value).toFixed(2)}`
      },
      {
        field: 'balance_due',
        headerName: 'Balance Due', flex: 1,
        valueFormatter: p => `₹${parseFloat(p.value).toFixed(2)}`
      },
      ...
      { field: 'payment_method', headerName: 'Payment Method', flex: 1 },
      {
        ...
        headerName: 'Payment Status', flex: 1,
        cellRenderer: (params: ICellRendererParams) => {
          ...
          return `<span style="padding: 2px 6px; font-size: 10px; font-weight: 600; color: #fff; background: ${color}; border-radius: 9999px;">${params.value}</span>`;
        }
      },
      {
        headerName: 'Receipt',
